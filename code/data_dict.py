...
def id_data(iid):
    ...
    tech = id_soup.find_all(
        "li", class_="organization__tag organization__tag--technology"
    )
    data_dict["tech"] = [x.text.strip() for x in tech]

    data_dict["org_type"] = id_soup.find(
        "li", class_="organization__tag organization__tag--category"
    ).text.strip()
    ...
    for student in id_soup.find_all("div", class_="pos-rel"):
        ...
        count += 1
        students.append([student.find("h2").text, project.text, project.get("ng-href")])
